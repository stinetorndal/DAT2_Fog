...
public class InquiryController {
    ...
    private void createInquiry(Context ctx, ConnectionPool connectionPool) {
        ...
        try {
            ...
            sendConfirmationEmail(customerInquiryPdf);
                /*Mailtrap opfatter to mails samtidig som spam. Der er kun en måde at løse det på,
                ved at kode en forsinkelse, men det kræver en ekstra catch, så hele blokken er kommenteret ud*/
            //  Thread.sleep(1500);

            sendEmailToFog(customerInquiryPdf);
    ...
    //Hjælpemetode til bodyText i email til Fog.
    private void sendEmailToFog(Customer customer) {
        String fogEmail = "salg@fog.dk";
        String subject = "Ny forespørgsel modtaget - Kundenavn: " + customer.getFirstname() + " " + customer.getLastname();
        //Body-tekst
        String bodyText = "Kære Fog-Salg\n\n"
                + "Du har modtaget en ny forespørgsel\n"
                + "Kig på Sælger-Dashboard for at se forespørgsel.\n";
        emailService.sendEmail(fogEmail, subject, bodyText);
    }
